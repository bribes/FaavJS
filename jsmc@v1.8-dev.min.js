...
function getSkyLevel(xp) {
  ...
  let exactLevel = 0
  if (xp >= 15000) {
    exactLevel = roundNumber((xp - 15000) / 10000 + 12, 2);
  } else {
    for (i = 0; i < xps.length; i++) {
      if (xp < xps[i]) {
        exactLevel = Math.floor(i + (xp - xps[i - 1]) / (xps[i] - xps[i - 1]));
        break;
      }
    }
  }
        ...
        player.type = e.status,
          ...
          fetch("https://playerdb.co/api/player/minecraft/" + a).then(a => a.json()).then(a => {
            ...
            player.username = a.data.player.username,
              ...
              player.name_history = a.data.player.meta.name_history.reverse(),
              ...
              fetch("https://api.hypixel.net/player?key=63f20355-92d7-42d7-a2a5-6457160aa4eb&uuid=" + player.id).then(a => a.json()).then(a => {
                player.servers.hypixel = new Object({}),
                  ...
                  null !== a.player && (player.servers.hypixel.rank = a.player.rank,
                    ...
                    player.servers.hypixel.stats = a.player.stats);
                if (player.servers.hypixel.stats.SkyWars !== undefined) {
                  player.servers.hypixel.stats.SkyWars.level = getSkyLevel(player.servers.hypixel.stats.SkyWars.skywars_experience);
                  player.servers.hypixel.stats.Bedwars.level = a.player.achievements.bedwars_level;
                };
              ...
              }).catch(a => player.error = true); //CATCHES ERROR
          });
      });
  });
}
